...
class Agent:
    ...
    def compute_neighbors(self):
        ...
        """
        rangeSq = rvo_math.square(
            self.time_horizon_obst_ * self.max_speed_ + self.radius_)
    ...
    def compute_new_velocity(self):
        ...
        for i in range(len(self.obstacle_neighbors_)):
            ...
            for j in range(len(self.orca_lines_)):
                det1 = rvo_math.det(invTimeHorizonObst * relativePosition1 -
                                    self.orca_lines_[j].point, self.orca_lines_[j].direction)
                det2 = rvo_math.det(invTimeHorizonObst * relativePosition2 -
                                    self.orca_lines_[j].point, self.orca_lines_[j].direction)
            ...
            s = (-relativePosition1 @ obstacleVector) / \
                rvo_math.abs_sq(obstacleVector)
            ...
            leftCutOff = invTimeHorizonObst * \
                (obstacle1.point_ - self.position_)
            rightCutOff = invTimeHorizonObst * \
                (obstacle2.point_ - self.position_)
            ...
            t = 0.5 if obstacle1 == obstacle2 else (
                (self.velocity_ - leftCutOff) @ cutOffVector) / rvo_math.abs_sq(cutOffVector)
            ...
            if distSqCutoff <= distSqLeft and distSqCutoff <= distSqRight:
                ...
                line.direction = -obstacle1.direction_
                line.point = leftCutOff + self.radius_ * invTimeHorizonObst * \
            ...
            line.point = rightCutOff + self.radius_ * invTimeHorizonObst * \
                Vector2(-line.direction.y, line.direction.x)
            self.orca_lines_.append(line)
        ...
        lineFail, self.new_velocity_ = self.linear_program2(
            self.orca_lines_, self.max_speed_, self.pref_velocity_, False, self.new_velocity_)

        if lineFail < len(self.orca_lines_):
            self.new_velocity_ = self.linear_program3(
                self.orca_lines_, numObstLines, lineFail, self.max_speed_, self.new_velocity_)
    ...
    def insert_agent_neighbor(self, agent, rangeSq):
        ...
        if self != agent:
            ...
            if distSq < rangeSq:
                ...
                if len(self.agent_neighbors_) == self.max_neighbors_:
                    rangeSq = self.agent_neighbors_[
                        len(self.agent_neighbors_) - 1][0]
        return rangeSq
    ...
    def insert_obstacle_neighbor(self, obstacle, rangeSq):
        ...
        distSq = rvo_math.dist_sq_point_line_segment(
            obstacle.point_, nextObstacle.point_, self.position_)
    ...
    def linear_program1(self, lines, lineNo, radius, optVelocity, directionOpt):
        ...
        discriminant = rvo_math.square(
            dotProduct) + rvo_math.square(radius) - rvo_math.abs_sq(lines[lineNo].point)
        ...
        for i in range(lineNo):
            denominator = rvo_math.det(
                lines[lineNo].direction, lines[i].direction)
            numerator = rvo_math.det(
    ...
    def linear_program2(self, lines, radius, optVelocity, directionOpt, result):
        ...
        for i in range(len(lines)):
            if rvo_math.det(lines[i].direction, lines[i].point - result) > 0.0:
                ...
                tempResult = result
                success, result = self.linear_program1(
